...
fn index_store_and_load_from_array() {
    ...
    assert!(
        has_array_gep || has_i32_gep,
        "expected GEPs for array indexing (either [3 x i32] or i32), IR:\n{}",
        ir
    );

    // Expect stores for a[0] and a[1]
    assert!(
        ir.contains("store i32 1, ptr"),
        "expected store of 1 into a[0], IR:\n{}",
        ir
    );
    assert!(
        ir.contains("store i32 2, ptr"),
        "expected store of 2 into a[1], IR:\n{}",
        ir
    );
    ...
    assert!(
        ...
        "expected loads from array elements, IR:\n{}",
        ir
    );
}
