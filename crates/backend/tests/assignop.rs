...
fn plus_assign_ident_ir_patterns() {
    ...
    assert!(
        ...
        "expected initial store of 5 to x, IR:\n{}",
        ir
    );
    ...
    assert!(
        ...
        "expected load of x before x+=3, IR:\n{}",
        ir
    );
    assert!(
...
fn and_assign_ident_ir_patterns() {
    ...
    assert!(
        ir.contains("store i32 6, ptr %x"),
        "init x=6 missing, IR:\n{}",
        ir
    );
    assert!(
        ir.contains("load i32, ptr %x"),
        "expected load of x before &=, IR:\n{}",
        ir
    );
    assert!(
        ir.contains("and i32"),
        "expected bitwise and for &=, IR:\n{}",
        ir
    );
    assert!(
        ir.contains("store i32"),
        "expected store after &=, IR:\n{}",
        ir
    );
}
...
fn mul_assign_deref_ir_patterns() {
    ...
    assert!(
        ...
        "expected load of pointee before *=, IR:\n{}",
        ir
    );
    assert!(
        ir.contains("mul i32") || ir.contains("mul nsw i32") || ir.contains("mul nuw i32"),
        "expected mul i32 for *=, IR:\n{}",
        ir
    );
    assert!(
        ir.contains("store i32"),
        "expected store back through pointer after *=, IR:\n{}",
        ir
    );
}
