...
fn comma_constant_rhs_returns_value() {
    ...
    assert!(ir.contains("ret i32 2"), "expected ret i32 2 in IR, got:\n{}", ir);
}
...
fn comma_assignment_then_value_stores_then_returns_rhs() {
    ...
    assert!(ir.contains("store i32 1, ptr %i"), "expected store of 1 into %i, IR:\n{}", ir);
    assert!(ir.contains("store i32 2, ptr %r"), "expected store of 2 into %r, IR:\n{}", ir);
