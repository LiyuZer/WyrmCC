...
fn pre_inc_ident_ir_patterns() {
    ...
    assert!(ir.contains("load i32, ptr %x"), "expected a load of x before increment, IR:\n{}", ir);
    assert!(
        ir.contains("add i32") || ir.contains("add nsw i32") || ir.contains("add nuw i32"),
        "expected an add i32 for ++x, IR:\n{}",
        ir
    );
    assert!(ir.contains("store i32"), "expected a store after increment, IR:\n{}", ir);
}
...
fn post_dec_ident_ir_patterns() {
    ...
    assert!(ir.contains("%x = alloca i32"), "expected alloca for x, IR:\n{}", ir);
    assert!(ir.contains("%y = alloca i32"), "expected alloca for y, IR:\n{}", ir);
    assert!(ir.contains("store i32 5, ptr %x"), "expected init of x, IR:\n{}", ir);
    assert!(ir.contains("load i32, ptr %x"), "expected load of x before post-decrement, IR:\n{}", ir);
    assert!(
    ...
    assert!(ir.contains("store i32"), "expected stores for y and updated x, IR:\n{}", ir);
}
...
fn pre_inc_deref_ir_patterns() {
    ...
    assert!(ir.contains("store ptr %x, ptr %p") || ir.contains("store ptr %"), "expected store of &x into p, IR:\n{}", ir);
    ...
    assert!(ir.contains("store i32"), "expected store back through pointer after increment, IR:\n{}", ir);
}
