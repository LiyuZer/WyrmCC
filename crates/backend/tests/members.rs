...
fn struct_dot_load_store() {
    ...
    assert!(
        ...
        "expected byte allocation for struct storage, IR:\n{}",
        ir
    );
    ...
    assert!(
        ...
        "expected store of 5 to struct field, IR:\n{}",
        ir
    );
    assert!(
        ir.contains("load i32"),
        "expected load from struct field, IR:\n{}",
        ir
    );
}
...
fn struct_arrow_store_then_read_from_base() {
    ...
    assert!(
        ...
        "expected i8 GEP for member access, IR:\n{}",
        ir
    );
    assert!(
        ir.contains("store i32 2"),
        "expected store 2 to q->a, IR:\n{}",
        ir
    );
    // And later load and return value from s.a
    assert!(
        ir.contains("load i32"),
        "expected load from s.a, IR:\n{}",
        ir
    );
}
...
fn union_dot_access_basic() {
    ...
    assert!(
        ...
        "expected store 7 to union field, IR:\n{}",
        ir
    );
    assert!(
        ir.contains("load i32"),
        "expected load from union field, IR:\n{}",
        ir
    );
}
