...
fn nested_struct_dot_access() {
    ...
    assert!(ir.contains("getelementptr inbounds i8"), "expected i8 GEP for nested member access, IR:\n{}", ir);
    // Expect stores of the constants
    assert!(ir.contains("store i32 3"), "expected store of 3 to s.in.a, IR:\n{}", ir);
    assert!(ir.contains("store i32 4"), "expected store of 4 to s.in.b, IR:\n{}", ir);
}
...
fn nested_struct_arrow_access() {
    ...
    assert!(ir.contains("getelementptr inbounds i8"), "expected i8 GEP for nested pointer member access, IR:\n{}", ir);
    assert!(ir.contains("store i32 5"), "expected store 5 to p->in.a, IR:\n{}", ir);
    assert!(ir.contains("store i32 6"), "expected store 6 to p->in.b, IR:\n{}", ir);
}
...
fn union_in_struct_overlay_offsets() {
    ...
    assert!(ir.contains(", i64 4") || ir.matches(", i64 ").count() > 1,
