...
fn pointers_addr_of_load() {
    ...
    // x is an i32 alloca
    assert!(ir.contains("%x = alloca i32"), "missing i32 alloca for x:\n{}", ir);
    // p is a ptr alloca
    assert!(ir.contains("%p = alloca ptr"), "missing ptr alloca for p:\n{}", ir);
    ...
    assert!(ir.contains("ret i32"), "expected integer return in IR:\n{}", ir);
}
...
fn pointers_store_through() {
    ...
    assert!(ir.contains("store ptr %x, ptr %p"), "missing store of &x into p:\n{}", ir);
    assert!(
    ...
    assert!(ir.contains(" add i32 ") || ir.contains("add i32"), "expected add instruction for x+1:\n{}", ir);
    assert!(ir.contains("ret i32"), "expected integer return in IR:\n{}", ir);
}
