...
fn switch_basic_ir_patterns() {
    ...
    assert!(ir.contains("ret i32 1") || ir.contains("ret i32 2"), "expected integer returns in IR:\n{}", ir);
}
...
fn switch_fallthrough_and_break_ir_shape() {
    ...
    assert!(ir.contains("icmp eq i32"), "expected equality compares in dispatch, IR:\n{}", ir);
    assert!(ir.contains("br i1 "), "expected conditional branches in dispatch, IR:\n{}", ir);

    // There should be some unconditional branches for fallthrough or break to the end label
    assert!(ir.contains("br label %L"), "expected some unconditional branches to labels (fallthrough/break), IR:\n{}", ir);

    // Heuristic: ensure multiple labels exist (dispatch + case blocks + end)
    assert!(count_substr(&ir, "\nL") >= 3, "expected at least 3 labeled blocks, IR:\n{}", ir);
}
