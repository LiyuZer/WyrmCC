...
fn shifts_and_bitwise() {
    ...
    writeln!(f, "int main(void) {{ return (1<<5) + (32>>2) + (6&3) + (6|1) + (6^2); }}").unwrap();
    let mut cmd = Command::cargo_bin("wyrmcc").unwrap();
    cmd.env("WYRMC_CLANG", clang).env("WYRMC_LLC", llc).args(["run", p.to_string_lossy().as_ref()]);
    cmd.assert().code(53);
}
...
fn comparisons_return_0_or_1() {
    ...
    writeln!(f, "int main(void) {{ return (3<4) + (5==5) + (5!=5) + (10>=7) + (2>7); }}").unwrap();
    let mut cmd = Command::cargo_bin("wyrmcc").unwrap();
    cmd.env("WYRMC_CLANG", clang).env("WYRMC_LLC", llc).args(["run", p.to_string_lossy().as_ref()]);
    cmd.assert().code(3);
}
...
fn precedence_mixed_bitwise_and_cmp() {
    ...
    writeln!(f, "int main(void) {{ return ((1|2)==3) + ((4&1)==1) + ((8^8)==0); }}").unwrap();
    let mut cmd = Command::cargo_bin("wyrmcc").unwrap();
    cmd.env("WYRMC_CLANG", clang).env("WYRMC_LLC", llc).args(["run", p.to_string_lossy().as_ref()]);
    cmd.assert().code(2);
}
