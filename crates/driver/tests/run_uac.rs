...
fn run_uac_int_vs_uint_and_conditional() {
    ...
    // Let a = -1 (int), b = 1 (unsigned). Then:
    //   a < b => false (since a converts to a big unsigned), so (a < b) ? a : b yields b => 1
    //   a > b => true, so (a > b) ? a : b yields a converted to unsigned => 0xFFFFFFFF (on 32-bit unsigned)
    ...
    unsigned int c = (a < b) ? a : b; // expect 1
    if (c != 1) return 11;

    unsigned int d = (a > b) ? a : b; // expect 0xFFFFFFFF
    if ((d + 1) != 0) return 12; // wrap-around check implies d == 0xFFFFFFFF
