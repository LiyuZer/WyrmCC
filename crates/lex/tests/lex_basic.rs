...
fn basic_c_tokens_sequence() {
    ...
    // y = 3 + 4 ;
    let mut i = 0;
    while i < toks.len() && !matches!(toks[i].kind, K::Identifier(ref s) if s == "y") { i += 1; }
    assert!(i + 5 < toks.len());
    assert!(matches!(toks[i].kind,     K::Identifier(ref s) if s == "y"));
    assert!(matches!(toks[i+1].kind,   K::Punct(P::Assign)));
    assert!(matches!(toks[i+2].kind,   K::Literal(LiteralKind::Int { .. })));
    assert!(matches!(toks[i+3].kind,   K::Punct(P::Plus)));
    assert!(matches!(toks[i+4].kind,   K::Literal(LiteralKind::Int { .. })));

    // if (y >= 7)
    let mut j = 0;
    while j < toks.len() && !matches!(toks[j].kind, K::Keyword(Keyword::If)) { j += 1; }
    assert!(j + 5 < toks.len());
    ...
    assert!(matches!(toks[j+4].kind,   K::Literal(LiteralKind::Int { .. })));
