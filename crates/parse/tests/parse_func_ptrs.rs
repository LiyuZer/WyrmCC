...
fn indirect_call_via_addr_of_function_parses_as_callptr() {
    ...
    let main_fn = tu.functions.iter().find(|f| f.name == "main").expect("main present");
        ...
        if let Stmt::Return(e) = s {
            match e {
                Expr::CallPtr { target, args } => {
                    if let Expr::Unary { op: UnaryOp::Deref, expr } = target.as_ref() {
                        if let Expr::Unary { op: UnaryOp::AddrOf, expr: inner } = expr.as_ref() {
