...
fn parse_basic_integer_specifiers_full_fidelity() {
    ...
    assert!(
        matches!(ty, Type::Short),
        "sh should be Short, got {:?}",
        ty
    );
...
fn parse_pointer_and_array_forms() {
    ...
    match typ {
        Type::Pointer(inner) => assert!(
            matches!(&**inner, Type::UInt),
            "pointer base should be UInt, got {:?}",
            inner
        ),
    ...
    match ty_arr {
        Type::Array(inner, n) => {
            assert!(
                matches!(&**inner, Type::Short),
                "array element type should be Short, got {:?}",
                inner
            );
...
fn parse_char_signedness_and_unsigned_widths() {
    ...
    assert!(
        matches!(ty, Type::SChar),
        "sc should be SChar, got {:?}",
        ty
    );

    let (_n, ty) = find_decl(&f.body, "uc").expect("uc decl");
    assert!(
        matches!(ty, Type::UChar),
        "uc should be UChar, got {:?}",
        ty
    );

    let (_n, ty) = find_decl(&f.body, "us").expect("us decl");
    assert!(
        matches!(ty, Type::UShort),
        "us should be UShort, got {:?}",
        ty
    );

    let (_n, ty) = find_decl(&f.body, "ul").expect("ul decl");
    assert!(
        matches!(ty, Type::ULong),
        "ul should be ULong, got {:?}",
        ty
    );
