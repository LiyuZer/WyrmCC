use parse::ast::Type;
use sema::{arith_result_type, cond_result_type, promoted_int_type, shift_result_lhs_promoted};
...
fn uac_key_combinations() {
    ...
    assert!(matches!(
        arith_result_type(&t(Type::Short), &t(Type::Short)),
        Some(Type::Int)
    ));

    // int vs uint => uint
    assert!(matches!(
        arith_result_type(&t(Type::Int), &t(Type::UInt)),
        Some(Type::UInt)
    ));
    assert!(matches!(
        arith_result_type(&t(Type::Short), &t(Type::UInt)),
        Some(Type::UInt)
    ));

    // long vs int => long
    assert!(matches!(
        arith_result_type(&t(Type::Long), &t(Type::Int)),
        Some(Type::Long)
    ));
    assert!(matches!(
        arith_result_type(&t(Type::Int), &t(Type::Long)),
        Some(Type::Long)
    ));

    // long vs uint (both 32-bit ranks here) => unsigned long
    assert!(matches!(
        arith_result_type(&t(Type::Long), &t(Type::UInt)),
        Some(Type::ULong)
    ));
    assert!(matches!(
        arith_result_type(&t(Type::UInt), &t(Type::Long)),
        Some(Type::ULong)
    ));

    // any with unsigned long => unsigned long
    assert!(matches!(
        arith_result_type(&t(Type::ULong), &t(Type::Int)),
        Some(Type::ULong)
    ));
    assert!(matches!(
        arith_result_type(&t(Type::ULong), &t(Type::UInt)),
        Some(Type::ULong)
    ));
    assert!(matches!(
        arith_result_type(&t(Type::Long), &t(Type::ULong)),
        Some(Type::ULong)
    ));
}
...
fn shift_result_is_promoted_lhs() {
    ...
    assert!(matches!(
        shift_result_lhs_promoted(&Type::Short),
        Some(Type::Int)
    ));
    assert!(matches!(
        shift_result_lhs_promoted(&Type::UShort),
        Some(Type::Int)
    ));

    // int << anything => int
    assert!(matches!(
        shift_result_lhs_promoted(&Type::Int),
        Some(Type::Int)
    ));

    // long << anything => long
    assert!(matches!(
        shift_result_lhs_promoted(&Type::Long),
        Some(Type::Long)
    ));
}
...
fn cond_operator_uses_uac_for_ints() {
    ...
    assert!(matches!(
        cond_result_type(&t(Type::Short), &t(Type::UInt)),
        Some(Type::UInt)
    ));

    // long : int => long
    assert!(matches!(
        cond_result_type(&t(Type::Long), &t(Type::Int)),
        Some(Type::Long)
    ));

    // ulong : int => ulong
    assert!(matches!(
        cond_result_type(&t(Type::ULong), &t(Type::Int)),
        Some(Type::ULong)
    ));

    // char : unsigned char => both promote to int => int
    assert!(matches!(
        cond_result_type(&t(Type::Char), &t(Type::UChar)),
        Some(Type::Int)
    ));
}
